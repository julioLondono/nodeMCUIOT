...
export const formatDataH = functions.database.ref('/DHT11/Humidity/{pushId}/')
    .onCreate(
        ...
        (snapshot, context) => {
            ...
            return admin.database().ref('timestamped_humidity').push({
                value: original,
...
export const formatDataT = functions.database.ref('/DHT11/Temperature/{pushId}/')
    .onCreate(
        (snapshot, context) => {
            const original = snapshot.val(); 
            const pushId = context.params.pushId; 
            console.log(`Detected new measure ${original} with pushId ${pushId}`);
            return admin.database().ref('timestamped_temperature').push({
                value: original,
                timestamp: admin.database.ServerValue.TIMESTAMP // will be Epoch time in ms
            });
        });
